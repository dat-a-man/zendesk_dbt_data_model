...
def incremental_load_all_default() -> Any:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="zendesk_pipeline1",
        destination='bigquery',
        full_refresh=False,
        dataset_name="sample_zendesk_data5",
    )
    ...
    info = pipeline.run(data=[data_chat, data_support, data_talk])
    return info
...
"""
curl https://{makepipelines}.zendesk.com/api/v2/oauth/clients.json \
  -X POST \
  -v -u {amancloudservices2@gmail.com}/token:{5LxDGAGDvgPPD0T3M5W1L1ZUtRj0pjGqSMQJ1Nfc} \
