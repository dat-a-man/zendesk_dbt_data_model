...
{% set active_ids_exist = adapter.get_relation(
        ...
        schema=this.schema,
        identifier="zendesk_pipeline1_dlt_processed_load_ids" ) %}
...
AND load_id NOT IN (SELECT load_id FROM {{ source('transformed_data', 'zendesk_pipeline1_dlt_processed_load_ids') }})
{% endif %}